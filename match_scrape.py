...
def match_scrape(num_pages):
    matches = []

    for page in range(1, num_pages + 1):
        print("Page: " + str(page) + f"/{num_pages}")
        ...
        for match in replays_data:
            id = match["id"]
            data = (
                requests.get(f"https://replay.pokemonshowdown.com/{id}.json").json()
            )["log"]
            ...
            matches.append({"p0 team": p0_team, "p1 team": p1_team, "winner": winner})
            time.sleep(1)
