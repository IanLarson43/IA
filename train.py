...
def train(file_name):
    ...
    train_set = lgb.Dataset(train_data, label=train_result_data)
    valid_set = lgb.Dataset(valid_data, label=valid_result_data)
    ...
    predictions = bst.predict(valid_data)
    prediction_labels = (predictions > 0.5).astype(int)
    print("Accuracy:", accuracy_score(valid_result_data, prediction_labels))
